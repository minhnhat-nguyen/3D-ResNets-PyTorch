...
import inference
from dataset import get_inference_data, get_training_data, get_validation_data
...
from utils import Logger, get_lr, worker_init_fn
...
def get_train_utils(opt, model_parameters):
    ...
    normalize = get_normalize_method(
        opt.mean, opt.std, opt.no_mean_norm, opt.no_std_norm
    )
    if not opt.no_hflip:
    ...
    if opt.is_master_node:
        train_logger = Logger(
            opt.result_path / "train.log", ["epoch", "loss", "acc", "lr"]
        )
...
def main_worker(index, opt):
    ...
    if opt.pretrain_path:
        model = load_pretrained_model(
            model, opt.pretrain_path, opt.model, opt.n_finetune_classes
        )
    if opt.resume_path is not None:
    ...
    if opt.tensorboard and opt.is_master_node:
        ...
        if opt.begin_epoch == 1:
            ...
            tb_writer = SummaryWriter(
                log_dir=opt.result_path, purge_step=opt.begin_epoch
            )
    else:
        tb_writer = None
    ...
    for i in range(opt.begin_epoch, opt.n_epochs + 1):
        if not opt.no_train:
            ...
            if i % opt.checkpoint == 0 and opt.is_master_node:
                ...
                save_checkpoint(
                    save_file_path, i, opt.arch, model, optimizer, scheduler
                )
