...
def extract_frames_smart_sampling(video_path, sample_duration=16, clips_per_second=1):
    ...
    print(
        f"Video has {total_frames} frames at {fps} FPS (duration: {duration_sec:.2f} seconds)"
    )
    ...
    if total_frames <= sample_duration:
        ...
        for clip_idx in range(target_clips):
            ...
            sample_indices = np.linspace(
                start_frame, end_frame - 1, sample_duration, dtype=int
            )
...
def predict_clips(model, clips, device, class_names, top_k=5):
    ...
    for clip_idx, clip in enumerate(clips):
        ...
        print(
            f"Clip {clip_idx+1} top prediction: {clip_predictions[0]['label']} ({clip_predictions[0]['score']:.4f})"
        )
...
def main():
    ...
    parser.add_argument(
        "--video", required=True, type=Path, help="Path to input video file"
    )
    parser.add_argument(
        "--checkpoint",
        default="saved/save_120.pth",
        type=Path,
    ...
    try:
        ...
        clips = extract_frames_smart_sampling(
            args.video, opt.sample_duration, args.clips_per_second
        )
