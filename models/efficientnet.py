...
class EfficientNet3D(nn.Module):
    ...
    def __init__(
        ...
        self.conv_head = nn.Conv3d(
            input_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False
        )
...
def get_efficientnet_params(version):
    ...
    if version in EFFICIENTNET_PARAMS:
        width_factor, depth_factor, resolution, dropout_rate = EFFICIENTNET_PARAMS[
            version
        ]
...
def generate_model(
    ...
    """
    width_factor, depth_factor, resolution, default_dropout_rate = (
        get_efficientnet_params(model_depth)
    )
